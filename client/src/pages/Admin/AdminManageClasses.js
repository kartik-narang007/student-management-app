import React from 'react';

const AdminManageClasses = () => {
    // Sample data
    const classes = [
        { id: 1, name: 'Math 101', teacher: 'Alice Johnson' },
        { id: 2, name: 'Science 202', teacher: 'Bob Brown' },
        // Add more classes as needed
    ];

    return (
        <div className="p-6">
            <h1 className="text-2xl font-bold">Manage Classes</h1>
            <div className="mt-4">
                <button className="bg-blue-500 text-white px-4 py-2 rounded mb-4">Add Class</button>
                <table className="min-w-full bg-white shadow rounded">
                    <thead>
                        <tr>
                            <th className="py-2 px-4 border-b">ID</th>
                            <th className="py-2 px-4 border-b">Class Name</th>
                            <th className="py-2 px-4 border-b">Teacher</th>
                            <th className="py-2 px-4 border-b">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {classes.map(cls => (
                            <tr key={cls.id}>
                                <td className="py-2 px-4 border-b">{cls.id}</td>
                                <td className="py-2 px-4 border-b">{cls.name}</td>
                                <td className="py-2 px-4 border-b">{cls.teacher}</td>
                                <td className="py-2 px-4 border-b">
                                    <button className="bg-yellow-500 text-white px-2 py-1 rounded">Edit</button>
                                    <button className="bg-red-500 text-white px-2 py-1 rounded ml-2">Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            </div>
        </div>
    );
};

export default AdminManageClasses;
